...
async function main() {
  let happyFundContract

  try {
    const contractAddresses = fs.readFileSync('./contracts/contractAddress.json', 'utf8')
    const { happyFundContract: happyFundAddress } = JSON.parse(contractAddresses)

    happyFundContract = await ethers.getContractAt('happyFund', happyFundAddress)

    // Process #1
    await Promise.all(
      generateCharities(charitiesCount).map(async (charity) => {
        await createCharity(happyFundContract, charity)
      })
    )
        ...
        .map(async (_, i) => {
          ...
          await Promise.all(
            supports.map(async (donation, i) => {
              await makeDonations(happyFundContract, i + 1, donation, {
                value: toWei(donation.amount),
              })
            })
          )
    ...
    // Process #3
    await getCharities(happyFundContract)
    await getSupports(happyFundContract, 1)
  } catch (error) {
